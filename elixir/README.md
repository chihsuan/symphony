...
- When `codex.turn_sandbox_policy` is set explicitly, Symphony forwards the configured map to
  ...
  `writableRoots` at runtime. For `workspace.strategy: worktree`, Symphony also includes the
  configured repository `.git` metadata root so branch, commit, and fetch operations can update the
  linked worktree. Compatibility for the remaining fields still depends on the targeted Codex
